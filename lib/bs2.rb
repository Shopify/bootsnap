module BS2
  ...
  def self.setup(return_false: [], guarantee_fail: [])
    ...
    if File.exist?('tmp/bs2.msgpack')
      puts 'loading'
      @@omg = MessagePack.load(File.binread('tmp/bs2.msgpack'))
    else
      puts 'generating'
      @@omg = [
        {},
      ...
      File.binwrite('tmp/bs2.msgpack', MessagePack.dump(@@omg))
    end

    require_relative 'bs2/core_ext'
  end

  # TODO: this simulates push/<<, not unshift
  def self.load_path_added(lpe)
    p = "tmp/bs2-#{Digest::MD5.hexdigest(lpe)}.msgpack"
    req = if File.exist?(p)
